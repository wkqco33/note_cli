...
API 서버 호출에 필요한 키는 바이너리에 포함하지 않고 **실행 시점**에 환경변수에서 읽습니다.

개발 환경에서는 저장소 루트의 `.env` 파일을 사용할 수 있습니다.
...
이미 셸이나 서비스 매니저에서 환경변수를 주입하고 있다면 `.env` 파일은 없어도 됩니다. 운영 환경에서는 `.env`보다 **시스템 환경변수, CI/CD secret, 서비스 매니저의 secret 주입 방식**을 권장합니다.
